...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'news_publication_status_check') THEN
    ALTER TABLE public.news ADD CONSTRAINT news_publication_status_check
...
DROP POLICY IF EXISTS "Admins can view RSS history" ON public.rss_sync_runs;
