...
import { Pencil, Trash2, Plus, Phone, Briefcase, MapPin } from "lucide-react";
...
export default function AdminServicos() {
  ...
  const handleSave = () => {
    if (!editingItem?.name || !editingItem?.phone || !editingItem?.neighborhood || !editingItem?.category) {
    ...
    <div className="p-4 md:p-8 max-w-6xl mx-auto pb-24">
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="max-w-2xl">
            ...
            <div className="grid gap-6 py-4">
              ...
              <Button onClick={handleSave} className="bg-orange-600 hover:bg-orange-700 font-black h-12">
                Salvar Cadastro
              </Button>
            </div>
          </DialogContent>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
        <Table>
          ...
          <TableBody>
            {services.length === 0 ? (
              <TableRow><TableCell colSpan={5} className="text-center py-12 text-slate-400">Nenhum profissional cadastrado.</TableCell></TableRow>
            ) : (
              services.map((item) => (
                <TableRow key={item.id}>
                  <TableCell>
                    <div className="font-bold">{item.name}</div>
                    <div className="text-xs text-slate-500 flex items-center gap-1"><Phone className="w-3 h-3" /> {item.phone}</div>
                  </TableCell>
                  <TableCell>
                    <Badge variant="outline" className="text-[10px] font-black uppercase">{item.category}</Badge>
                  </TableCell>
                  <TableCell>
                    <div className="text-sm font-medium">{item.neighborhood}</div>
                  </TableCell>
                  <TableCell>
                    <div className="flex items-center gap-2">
